...
(function()
{
      ...
      GameHandler.bitmaps.execute(
         function(percentComplete) {
            ...
            if (firstCallback)
            {
               ...
               Game.fillText(ctx, "Mohon Tunggu...", "14pt Courier New", offx, offy-6, "lightgrey");
               firstCallback = false;
            }
         },
         function() {
   ...
   extend(Arena.AttractorScene, Game.Scene,
   {
      ...
      sceneRendererWelcome: function sceneRendererWelcome(ctx)
      {
         ...
         Game.centerFillText(ctx, "Mulon", ff, GameHandler.height*0.5 - 24, "white");
         ...
         Game.centerFillText(ctx, "Klik SPASI Untuk Mulai", ff, GameHandler.height*0.5 + 12, colour);
         
         ff = ~~(10 * this.world.scale * 2) + "pt Arial";
         Game.fillText(ctx, "Multimedia Online", ff, 10, GameHandler.height-(10 * this.world.scale * 2), "white");
      },
      ...
      sceneRendererInfo: function sceneRendererInfo(ctx)
      {
         ...
         ctx.fillText("Bagaimana cara bermain...", GameHandler.width * 0.2, ypos);
         ctx.font = ~~(12 * this.world.scale * 2) + "pt Arial";
         ypos += ~~(32 * this.world.scale * 2);
         ctx.fillText("A W S D - Tombol Navigasi.", GameHandler.width * 0.15, ypos);
         ypos += ~~(20 * this.world.scale * 2);
         ctx.fillText("T - Untuk mematikan suara.", GameHandler.width * 0.15, ypos);
         ypos += ~~(20 * this.world.scale * 2);
         ctx.fillText("[ ] - Zoom papan main.", GameHandler.width * 0.15, ypos);
         ypos += ~~(20 * this.world.scale * 2);
         ctx.fillText("ESC - Pause game.", GameHandler.width * 0.15, ypos);
      },
...
(function()
{
   Arena.GameScene = function(game)
   {
      ...
      var interval = new Game.Interval("Mulai Game!", this.intervalRenderer);
